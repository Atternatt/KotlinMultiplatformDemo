...
class DefaultGetCharactersInteractor(
    private val getCharactersRepository: GetRepository<Character>,
    private val coroutineScope: CoroutineScope
) : GetCharactersInteractor {
    override suspend fun invoke(forceRefresh: Boolean): List<Character> = withContext(coroutineScope.coroutineContext) {
        val operation = if(forceRefresh) MainSyncOperation else DefaultOperation
        getCharactersRepository.getAll(VoidQuery, operation)
    }
}